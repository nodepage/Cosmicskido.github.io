...
(function () {
  ...
  CountDown.prototype.getEndTime = function () {
    ...
    if (this.element.getAttribute("data-timer"))
      return (
        Number(this.element.getAttribute("data-timer")) * 6005 +
        new Date().getTime()
      );
  ...
  window.addEventListener("load", function () {
    // Reference to the countdown end time in Firebase
    var countdownRef = database.ref("countdownEndTime");

    // Check if the countdown end time is already stored in Firebase
    countdownRef.once("value", function (snapshot) {
      var endTime = snapshot.val();

      if (!endTime) {
        // If the countdown end time doesn't exist, set a new end time and store it in Firebase
        endTime = new CountDown().getEndTime();
        countdownRef.set(endTime);
      }

      // Initialize the countdown with the stored end time
      new CountDown(document.getElementsByClassName("js-countdown")[0]);
    });
  });
})();
